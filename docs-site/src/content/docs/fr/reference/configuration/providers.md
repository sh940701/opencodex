...
| `noStopModels?` | `string[]` | Modèles qui rejettent `stop` / `stop_sequences` spécifié par l’appelant. L’adaptateur openai-chat et le passthrough Chat Completions omettent `stop` pour ces modèles (xAI grok-4.6 répond 400 invalid-argument « Model grok-4.6 does not support parameter stop. », ce qui casse le classificateur de sécurité auto-mode de Claude Code). |
